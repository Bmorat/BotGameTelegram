...
public class Nave3J2Handler : BaseHandler
{
    ...
    public Nave3J2Handler(IHandler next) : base(next)
    {
        this.Keywords = new string[] { "B3" };
    }

    protected override void InternalHandle(Message message, out string response)
    {

        if (message.Text.StartsWith("B3"))
        {
            string mensaje = message.Text;
            string substr1 = mensaje.Substring(3);
            RegistrarNave(substr1, 4);

            if (GlobalLibrary.NavesJ1.Count > 0)
            {
            ...
            else
            {
                ...
                {
                    TelegramBot.SendTextMessageAsync("Partida Encontrada! \nConfiguracion: Set standard (tablero 10x10 con 3 Naves; 2 de 3 de vida y 1 de 4 de vida) \nIngresa la posicion de tu primer nave con el formato: 'N1 XYXY' donde los primeros XY son la coordenada inicial de tu barco y los segundos XY son la coordenada final de tu barco:", GlobalLibrary.JugadoresJugando[0].Id);
                    response = string.Empty;
                }
                else
                {
                    TelegramBot.SendTextMessageAsync("Partida Encontrada! \nConfiguracion: Configuracion: Set mini (tablero 6x6 con 2 naves de 2 de 3 vida) \nIngresa la posicion de tu primer nave con el formato: 'B1 XYXY' donde los primeros XY son la coordenada inicial de tu barco y los segundos XY son la coordenada final de tu barco:", GlobalLibrary.JugadoresJugando[0].Id);
                    response = string.Empty;
                }
            }
        }
        ...
        {
            response = "Comando incorrecto, respete el formato 'BN XYXY'";
        }
    }
}